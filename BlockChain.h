...
struct Block {
 ...
 string time;
public:
 Block(const Transaction& transaction, const string& timestamp, Block* next = nullptr):
        next(next), info(transaction), time(timestamp){
 }
 ~Block(){delete next;}
};
...
struct BlockChain {
 ...
 //BlockChain* prev;
 Block* block;
 BlockChain():block(nullptr){}

 ~BlockChain(){delete block;}
};
